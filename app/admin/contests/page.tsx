...
// Upload a promo video to Cloudinary (same unsigned preset used elsewhere).
...
function ContestForm({ contest, tracks, onSave, onCancel }) {
  ...
  return (
    <div style={{ ...s.card, border: '1px solid var(--accent-primary)' }}>
      ...
      <div style={s.divider} />
      <div style={{ fontSize: '13px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Promo video</div>
      ...
      {form.promo_video_url ? (
        ...
        <label style={{ color: 'var(--accent-primary)', fontSize: '13px', fontWeight: 600, cursor: videoPct != null ? 'default' : 'pointer', display: 'inline-block', marginBottom: '10px' }}>
          {videoPct != null ? `Uploading… ${videoPct}%` : '⬆ Upload a promo video'}
          <input type="file" accept="video/*" style={{ display: 'none' }} disabled={videoPct != null} onChange={e => e.target.files[0] && doUploadVideo(e.target.files[0])} />
        </label>
      )}
